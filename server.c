...
//    using fcntl and O_NONBLOCK
// What sockets are made non-blocking?
//    the server socket to accept client connections, and the server socket to
//    receive client messages
// Why are these sockets made non-blocking? Waht purpose does it serve?
//    you can be running multiple client requests at the same time
...
int main() {
  ...
  while (true) {
    ...
    if (aargs.list_handle->count >= (MAX_CLIENTS * NUM_MSG_PER_CLIENT)) {
